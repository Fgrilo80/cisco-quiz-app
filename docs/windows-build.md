...
Version **1.2.10+12**, bundled bank **1230** questions in `assets/cricket.json` (Cricket `main` `02c0a868`, post length-bias). Do not edit the quiz bank to produce this binary.
...
| Version | `1.2.10+12` (`pubspec.yaml`) |
| Bundled bank | `assets/cricket.json`, **1230** questions |
| Release | https://github.com/Fgrilo80/cisco-quiz-app/releases/tag/v1.2.10 |
...
- Triggers: manual `workflow_dispatch`; push to `app` or `cursor/**` when `pubspec.yaml`, the bank, or the Windows project changes; push of tag `v1.2.10`. Pushes to `cursor/**` run so a pull request can publish the zip before it merges.
- Checkout uses the ref that started the run (including tag `v1.2.10`).
- Steps: Flutter 3.47.2, `flutter config --enable-windows-desktop`, `flutter pub get`, `flutter build windows --release`.
- The job refuses to publish unless `pubspec.yaml` is `1.2.10+12` and the bundled bank still has 1230 `"question"` entries.
- Zip: contents of `build/windows/x64/runner/Release` (so `cisco_quiz.exe`, DLLs, and `data\` sit at the root of the archive) as `CiscoQuiz-1.2.10-Windows.zip`.
- Artifact name: `CiscoQuiz-1.2.10-Windows` (workflow run → Artifacts).
- Release asset: https://github.com/Fgrilo80/cisco-quiz-app/releases/download/v1.2.10/CiscoQuiz-1.2.10-Windows.zip
- If release `v1.2.10` does not exist yet, the job creates it on the triggering commit and then uploads the zip. `--clobber` replaces only an asset of the same name.

Manual run after the workflow file is on the branch:

```bash
gh workflow run windows-release.yml --ref cursor/ota-hash-windows-1.2.10-6ea2
```
...
2. Checks version `1.2.10+12` and bank count 1230.
...
5. Writes `%USERPROFILE%\Desktop\CiscoQuiz-1.2.10-Windows.zip` (top-level folder `CiscoQuiz`, unlike the CI zip which has the Release files at the archive root).
